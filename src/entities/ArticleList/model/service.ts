...
export const getArticlesMore = createAsyncThunk<
	void,
	void,
	ThunkConfig<{title: string; text: string}>
>('articles/getArticlesMore', (_, {dispatch, getState}) => {
	const state = getState();
	const {hasMore, status, page} = state.articles || {
		limit: 1,
		hasMore: false,
	};
...
export const initArticles = createAsyncThunk<
	...
	URLSearchParams,
	ThunkConfig<{title: string; text: string}>
>('articles/initArticles', async (urlParams, {dispatch, getState}) => {
	const state = getState();
