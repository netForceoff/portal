...
const ArticlesPageFilters: FC<IArticlesPageFiltersProps> = (props) => {
  ...
  const handleChangeOptionSort = useCallback((value: SortBy) => {
    dispatch(articleListActions.setPage(1))
    dispatch(articleListActions.setSort(value))
    filter()
  }, [dispatch, filter])

  const handleChangeOptionOrder = useCallback((value: Order) => {
    dispatch(articleListActions.setPage(1))
  ...
  return (
    <div className={clsx(className)}>
        <Select<SortBy>
          label={t('sortBy')} options={optionsSort} onChange={handleChangeOptionSort} value={sortBy} />
        <Select label={t('sortBy')} options={optionsOrder} onChange={handleChangeOptionOrder} value={order} />
