...
export const updateProfile = createAsyncThunk<Profile, string | undefined, ThunkConfig<{ title: string, text: string }>>(
  'profile/updateProfile',
  async (id, { extra, dispatch, rejectWithValue, getState }) => {
    try {
      const profile = getProfileState(getState())
      const response = await extra.axiosApi.put<Profile>(`/profiles/${id || ''}`, profile)
