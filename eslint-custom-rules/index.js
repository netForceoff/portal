	...
	rules: {
		'relative-paths': {
			...
			meta: {
				docs: {
					description:
						'Правило для использования относительных путей в импортах',
				},
				fixable: 'code',
				schema: [
					{
		...
		'public-api': {
			...
			meta: {
				docs: {
					description:
						'Правило для использования PUBLIC API в файлах по FSD системе',
				},
				fixable: 'code',
				schema: [
					{
		...
		'fsd-architecture': {
			...
			meta: {
				docs: {
					description:
						'Правило для соблюдения корректного импорта по методологии FSD',
				},
				schema: [
					{
						type: 'object',
